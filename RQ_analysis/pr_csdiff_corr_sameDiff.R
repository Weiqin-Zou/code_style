...
pr_corr<-function(pr_d){
    pr_glm<-glm(is_merged~commit_num+churn_total_size+churn_final_size+churn_max_size+churn_file_num+final_churn_file_num+sameDiff,data=pr_d,family=binomial(link="logit"))
    print("pr logi lm for is_merged:")
    print(summary(pr_glm))

    print("pr lm for is_merged:")
    pr_lm<-lm(is_merged~commit_num+churn_total_size+churn_final_size+churn_max_size+churn_file_num+final_churn_file_num+sameDiff,pr_d)
    print(summary(pr_lm))

    print("pr lm for time_close:")
    pr_lm<-lm(time_close~commit_num+churn_total_size+churn_final_size+churn_max_size+churn_file_num+final_churn_file_num+sameDiff,pr_d)
    print(summary(pr_lm))
}
...
proj_corr<-function(proj_d){
    ...
    proj_lm<-lm(isMerge~proj_loc+proj_age+proj_star+proj_fork+proj_open_pr+proj_insider+sameDiffave,proj_d)
    print(summary(proj_lm))

    print("proj level multi logi for is_merged:")
    proj_lm<-lm(closeTimeave~proj_loc+proj_age+proj_star+proj_fork+proj_open_pr+proj_insider+sameDiffave,proj_d)
    print(summary(proj_lm))
}
